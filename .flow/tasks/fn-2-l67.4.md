...
## Done summary
Ported flow-plan-review and flow-impl-review skills to Factory format with semantic descriptions, Verification sections using 4-backtick format, dynamic repo root paths with graceful degradation, and removed hooks dependencies while preserving Carmack-level review standards.
## Evidence
- Commits: f46e87850fd63a31cdadea5e02202cb9e4beab2e
- Tests: ls verification for SKILL.md files, grep verification for semantic descriptions, grep verification for 4-backtick format, grep verification for dynamic repo root, grep verification for no hooks dependency, grep verification for flowctl rp commands, grep verification for Carmack-level standards
- PRs: